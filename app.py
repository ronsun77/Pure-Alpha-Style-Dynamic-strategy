...
# 0. 網頁基礎設定與終極 CSS
# ==========================================
st.set_page_config(page_title="Pure Alpha 戰情室 V8.0", layout="wide")
...
# 計算 MA200 與全歷史 Regime
df_all['MA200'] = df_all['QQQ'].rolling(200).mean()
is_bull_hist = df_all['QQQ'] >= (df_all['MA200'] * 0.97)
...
# 提取最新一天的狀態
latest_idx = df_all.index[-1]
is_bull = is_bull_hist.loc[latest_idx]
targets = (tgt_weights_df.loc[latest_idx] * 100).to_dict()

# 儀表板當前數據
sim_qqq = df_all["QQQ"].loc[latest_idx]
sim_ma200 = df_all["MA200"].loc[latest_idx]
cutoff_line = sim_ma200 * 0.97
ratio = sim_qqq / sim_ma200 if sim_ma200 > 0 else 1.0
if sim_qqq >= sim_ma200: regime_text, r_class = "核心進攻模式", "bull-box"
elif sim_qqq >= cutoff_line: regime_text, r_class = "多頭破位警戒區", "neutral-box"
else: regime_text, r_class = "熊市冬眠啟動", "bear-box"

# ==========================================
# 3. 儀表板 UI 上半部
# ==========================================
st.markdown("<h1 style='color:white;'>Pure Alpha 戰情室 V8.0</h1>", unsafe_allow_html=True)
...
with col_pie:
    ...
    fig_pie = go.Figure(data=[go.Pie(
        labels=list(CURRENT_WEIGHTS.keys()), values=list(CURRENT_WEIGHTS.values()), hole=.45, 
        textinfo='label+percent', marker=dict(colors=[CHART_COLORS[l] for l in CURRENT_WEIGHTS.keys()], line=dict(color='#081028', width=2))
    )])
...
with col_beta:
    st.markdown(f"<h2 style='color: #38bdf8; font-size: 18px; border-left: 4px solid #38bdf8; padding-left: 10px; margin-bottom: 10px;'>動態 Beta 趨勢 (即時還原歷史權重)</h2>", unsafe_allow_html=True)
    fig_beta = go.Figure()
    ...
    # 關鍵修正：將每天的 Rolling Beta 乘上「當天歷史策略規定的目標權重」
    # 這樣在 QQQ 破線當天，QLD 權重瞬間歸零，實倉組合的 Beta 也會瞬間真實暴跌！
    ...
    fig_beta.add_trace(go.Scatter(
        x=port_beta_dynamic.index, y=port_beta_dynamic, mode='lines',
        name='策略組合 (Dynamic)', line=dict(color='#22c55e', width=3.5)
    ))
...
if len(bt_df) > 10:
    ...
    for i in range(n_days):
        day_ret = ret_array[i]
        daily_p_ret = np.dot(current_w, day_ret) # 當日淨值報酬
        
        # 結算今日淨值
        port_nav[i] = (1.0 * (1 + daily_p_ret)) if i == 0 else (port_nav[i-1] * (1 + daily_p_ret))
        
        # 結算後各資產權重產生自然漂移 (Drift)
        drifted_w = current_w * (1 + day_ret) / (1 + daily_p_ret)
        
        if i < n_days - 1:
            tgt_w = tgt_array[i+1] # 明天的目標權重 (受 Regime 控制)
            
            # 判斷漂移是否大於門檻，或是否發生牛熊轉換(目標權重劇變)
            deviations = np.abs(drifted_w - tgt_w)
            if np.max(deviations) >= threshold_frac:
                current_w = tgt_w.copy() # 執行再平衡
                rebalance_count += 1
            else:
                current_w = drifted_w.copy() # 不作為，繼續放任漂移
    ...
    <div style="background: rgba(23, 35, 58, 0.5); padding: 20px; border-radius: 12px; margin-top: 20px; border-left: 5px solid #38bdf8;">
        ...
        <div class="report-text">
            <p>在選定的區間內，多頭進攻期共 <b>{bull_days}</b> 天，觸發空頭冬眠防禦共 <b>{bear_days}</b> 天。</p>
